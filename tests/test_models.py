...
def test_rent_is_returned(app):
    with app.app_context():
        ...
        rent_date = datetime.utcnow() - timedelta(days=5)  # Rent date 5 days ago
        return_date = datetime.utcnow() - timedelta(days=2)  # Return date 2 days ago
        test_rent = Rent(dressId=dress.id, clientId=customer.id, rentDate=rent_date, returnDate=return_date)
        ...
        rent_date = datetime.utcnow() - timedelta(days=5)  # Rent date 5 days ago
        return_date = datetime.utcnow() + timedelta(days=2)  # Return date 2 days from now
        test_rent = Rent(dressId=dress.id, clientId=customer.id, rentDate=rent_date, returnDate=return_date)
